...
async function getData(){
    const carsDataRawSource = await fetch('https://storage.googleapis.com/tfjs-tutorials/carsData.json')
    const carsDataProcessing = await carsDataRawSource.json();
    const carsDataClaned = carsDataProcessing.map(car => ({
        kmpg: car.Miles_per_Gallon, 
        horsepower: car.Horsepower,
    }))
    //Remove any pre-defined data that doesn't contain kmpg and horsepower data units
    .filter(car => (
        car.kmpg != null && car.horsepower != null
    ));
